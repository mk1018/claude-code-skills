...
1. **ブランチの作成**
   ...
   - staging ブランチから新しいブランチを作成（必要に応じて staging を最新化）

2. **ブランチの確認（必須）**
   - `git branch --show-current` で現在のブランチを確認する
   - **`main` または `staging` にいる場合は絶対にコミット・プッシュしない**。必ず新しいブランチを作成してから作業すること
...
- staging ブランチがベースブランチであることを確認する
